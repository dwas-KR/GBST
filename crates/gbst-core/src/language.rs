...
pub fn t(language: LanguageOption, key: &str) -> &'static str {
    ...
    match key {
        ...
        "open_apk_links" => if ko { "GitHub APK 링크 열기" } else { "Open GitHub APK Links" },
...
fn translate_dynamic_runtime_text(language: LanguageOption, text: &str) -> Option<String> {
    ...
    if text == "APK 다운로드에 실패했습니다. GitHub APK 링크 파일 또는 GitHub Release APK 다운로드 링크를 다시 확인해주세요." {
...
fn translate_korean_log_fragments(language: LanguageOption, text: &str) -> Option<String> {
    ...
    let replacements = [
        ...
        ("GitHub APK 링크 파일 다운로드 실패", lang_text(language, "failed to download the GitHub APK link file", "не удалось загрузить файл ссылок APK GitHub", "GitHub APKリンクファイルのダウンロードに失敗しました", "GitHub APK 連結檔案下載失敗", "tải tệp liên kết APK GitHub thất bại", "αποτυχία λήψης αρχείου συνδέσμων APK GitHub", "GitHub APK लिंक फ़ाइल डाउनलोड विफल", "GitHub APK ბმულების ფაილის ჩამოტვირთვა ვერ მოხერხდა", "GitHub APK-linkbestand downloaden mislukt", "فشل تنزيل ملف روابط APK من GitHub", "falló la descarga del archivo de enlaces APK de GitHub")),
        ("GitHub APK 링크 파일 읽기 실패", lang_text(language, "failed to read the GitHub APK link file", "не удалось прочитать файл ссылок APK GitHub", "GitHub APKリンクファイルの読み取りに失敗しました", "GitHub APK 連結檔案讀取失敗", "đọc tệp liên kết APK GitHub thất bại", "αποτυχία ανάγνωσης αρχείου συνδέσμων APK GitHub", "GitHub APK लिंक फ़ाइल पढ़ना विफल", "GitHub APK ბმულების ფაილის წაკითხვა ვერ მოხერხდა", "GitHub APK-linkbestand lezen mislukt", "فشل قراءة ملف روابط APK من GitHub", "falló la lectura del archivo de enlaces APK de GitHub")),
...
fn phrase(language: LanguageOption, key: &str) -> Option<&'static str> {
    Some(match key {
        ...
        "apk_download_failed" => lang_text(language, "APK download failed. Please check the GitHub APK link file or the GitHub Release APK download links.", "Не удалось загрузить APK. Проверьте файл ссылок APK на GitHub или ссылки загрузки APK из GitHub Release.", "APKのダウンロードに失敗しました。GitHubのAPKリンクファイルまたはGitHub ReleaseのAPKダウンロードリンクを確認してください。", "APK 下載失敗。請確認 GitHub APK 連結檔案或 GitHub Release APK 下載連結。", "Tải APK thất bại. Vui lòng kiểm tra tệp liên kết APK trên GitHub hoặc liên kết tải APK của GitHub Release.", "Η λήψη APK απέτυχε. Ελέγξτε το αρχείο συνδέσμων APK στο GitHub ή τους συνδέσμους λήψης APK του GitHub Release.", "APK डाउनलोड विफल। GitHub APK लिंक फ़ाइल या GitHub Release APK डाउनलोड लिंक जाँचें।", "APK ჩამოტვირთვა ვერ მოხერხდა. შეამოწმეთ GitHub APK ბმულების ფაილი ან GitHub Release APK ჩამოტვირთვის ბმულები.", "APK-download mislukt. Controleer het GitHub APK-linkbestand of de GitHub Release APK-downloadlinks.", "فشل تنزيل APK. تحقق من ملف روابط APK على GitHub أو روابط تنزيل APK في GitHub Release.", "Error al descargar APK. Revisa el archivo de enlaces APK de GitHub o los enlaces de descarga APK de GitHub Release."),
