...
pub fn t(language: LanguageOption, key: &str) -> &'static str {
    ...
    match key {
        ...
        "open_apk_links" => if ko { "원격 APK catalog 열기" } else { "Open Remote APK Catalog" },
...
fn translate_dynamic_runtime_text(language: LanguageOption, text: &str) -> Option<String> {
    ...
    if text == "[ADB] 업데이트(업데이트) 알림 및 권한을 비활성화 합니다." {
        return Some(phrase(language, "update_notice_redacted")?.to_string());
    }
    ...
    if text == "[안내]  PC(노트북), 케이블, 기기를 가만히 둔 상태로 1분~5분 대기해 주세요."
        || text == "[안내] PC(노트북), 케이블, 기기를 가만히 둔 상태로 1분~5분 대기해 주세요."
    {
    ...
    if text == "APK 다운로드에 실패했습니다. 원격 APK catalog 파일 또는 APK 다운로드 링크를 다시 확인해주세요." {
...
fn translate_korean_log_fragments(language: LanguageOption, text: &str) -> Option<String> {
    ...
    let replacements = [
        ...
        ("원격 APK catalog 파일 다운로드 실패", lang_text(language, "failed to download the remote APK catalog file", "не удалось загрузить файл ссылок GitHub APK catalog", "GitHub APK catalogリンクファイルのダウンロードに失敗しました", "GitHub APK catalog 連結檔案下載失敗", "tải tệp liên kết GitHub APK catalog thất bại", "αποτυχία λήψης αρχείου συνδέσμων GitHub APK catalog", "GitHub APK catalog लिंक फ़ाइल डाउनलोड विफल", "GitHub APK catalog ბმულის ფაილის ჩამოტვირთვა ვერ მოხერხდა", "remote APK catalog file downloaden mislukt", "فشل تنزيل ملف روابط GitHub APK catalog", "falló la descarga del archivo de enlaces GitHub APK catalog")),
        ("원격 APK catalog 파일 읽기 실패", lang_text(language, "failed to read the remote APK catalog file", "не удалось прочитать файл ссылок GitHub APK catalog", "GitHub APK catalogリンクファイルの読み取りに失敗しました", "GitHub APK catalog 連結檔案讀取失敗", "đọc tệp liên kết GitHub APK catalog thất bại", "αποτυχία ανάγνωσης αρχείου συνδέσμων GitHub APK catalog", "GitHub APK catalog लिंक फ़ाइल पढ़ना विफल", "GitHub APK catalog ბმულის ფაილის წაკითხვა ვერ მოხერხდა", "remote APK catalog file lezen mislukt", "فشل قراءة ملف روابط GitHub APK catalog", "falló la lectura del archivo de enlaces GitHub APK catalog")),
        ("APK catalog 처리 실패", lang_text(language, "APK catalog processing failed", "сбой обработки APK catalog", "APK catalog処理失敗", "APK catalog 處理失敗", "xử lý APK catalog thất bại", "αποτυχία επεξεργασίας APK catalog", "APK catalog प्रोसेसिंग विफल", "APK catalog-ის დამუშავება ვერ მოხერხდა", "APK catalog verwerken mislukt", "فشل معالجة APK catalog", "falló la procesamiento APK catalog")),
        ("APK catalog 처리 결과가 UTF-8 텍스트가 아닙니다", lang_text(language, "APK catalog processing result is not UTF-8 text", "результат обработки APK catalog не является текстом UTF-8", "APK catalog処理結果がUTF-8テキストではありません", "APK catalog 處理結果不是 UTF-8 文字", "kết quả xử lý APK catalog không phải văn bản UTF-8", "το αποτέλεσμα επεξεργασίας APK catalog δεν είναι κείμενο UTF-8", "APK catalog प्रोसेसिंग परिणाम UTF-8 टेक्स्ट नहीं है", "APK catalog დამუშავებული შედეგი UTF-8 ტექსტი არ არის", "APK catalog-geprocesserd resultaat is geen UTF-8-tekst", "نتيجة معالجة APK catalog ليست نص UTF-8", "el resultado procesado de APK catalog no es texto UTF-8")),
...
fn phrase(language: LanguageOption, key: &str) -> Option<&'static str> {
    Some(match key {
        ...
        "update_notice_redacted" => lang_text(language, "[ADB] Disabling 업데이트 (update) notifications and permissions.", "[ADB] Отключение уведомлений и разрешений 업데이트 (обновления).", "[ADB] 업데이트（更新）通知と権限を無効化します。", "[ADB] 正在停用 업데이트（更新）通知與權限。", "[ADB] Đang tắt thông báo và quyền 업데이트 (cập nhật).", "[ADB] Απενεργοποίηση ειδοποιήσεων και δικαιωμάτων 업데이트 (ενημέρωση).", "[ADB] 업데이트 (अपडेट) सूचनाएँ और अनुमतियाँ बंद की जा रही हैं।", "[ADB] 업데이트 (განახლება) შეტყობინებები და ნებართვები ითიშება.", "[ADB] 업데이트-meldingen (updates) en machtigingen uitschakelen.", "[ADB] جارٍ تعطيل إشعارات وأذونات 업데이트 (التحديث).", "[ADB] Desactivando notificaciones y permisos 업데이트 (actualización)."),
        ...
        "apk_download_failed" => lang_text(language, "APK download failed. Please check the remote APK catalog file or the APK download links.", "Не удалось загрузить APK. Проверьте файл ссылок GitHub APK catalog или обработанные ссылки APK.", "APKのダウンロードに失敗しました。GitHub APK catalogリンクファイルまたは処理されたAPKダウンロードリンクを確認してください。", "APK 下載失敗。請確認 GitHub APK catalog 連結檔案或處理後的 APK 下載連結。", "Tải APK thất bại. Vui lòng kiểm tra tệp liên kết GitHub APK catalog hoặc các liên kết APK đã xử lý.", "Η λήψη APK απέτυχε. Ελέγξτε το αρχείο συνδέσμων GitHub APK catalog ή τους επεξεργασμένους συνδέσμους APK.", "APK डाउनलोड विफल। कृपया GitHub APK catalog लिंक फ़ाइल या प्रोसेस किए गए APK डाउनलोड लिंक जाँचें।", "APK ჩამოტვირთვა ვერ მოხერხდა. შეამოწმეთ GitHub APK catalog ბმულების ფაილი ან დამუშავებული APK ბმულები.", "APK-download mislukt. Controleer het remote APK catalog file of de geprocesserde APK-downloadlinks.", "فشل تنزيل APK. تحقق من ملف روابط GitHub APK catalog أو روابط APK بعد المعالجة.", "Error al descargar APK. Revisa el archivo de enlaces GitHub APK catalog o los enlaces APK procesados."),
        ...
        "err_adb_timeout" => lang_text(language, "ADB device detection timed out. Last state:", "Время обнаружения ADB-устройства истекло. Последнее состояние:", "ADB端末検出がタイムアウトしました。最後の状態:", "ADB 裝置偵測逾時。最後狀態：", "Phát hiện thiết bị ADB đã hết thời gian. Trạng thái cuối:", "Έληξε ο χρόνος εντοπισμού συσκευής ADB. Τελευταία κατάσταση:", "ADB डिवाइस पहचान समय समाप्त। अंतिम स्थिति:", "ADB მოწყობილობის აღმოჩენის დრო ამოიწურა. ბოლო მდგომარეობა:", "Time-out bij ADB-apparaatdetectie. Laatste status:", "انتهت مهلة اكتشاف جهاز ADB. آخر حالة:", "Tiempo agupdatedo al detectar dispositivo ADB. Último estado:"),
